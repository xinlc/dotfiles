...
if (!__vieb_custom_deleteToStart) {
    /**
     * @param {HTMLElement} el
     */
    function __vieb_custom_deleteToStart(el) {
        if (el.value) {
            const match = el.value.slice(0, el.selectionStart).match(/\n.*/gi);
            const index = match ? el.value.lastIndexOf(match[match.length-1], el.selectionStart) + 1 : 0;
            ...
            s.setBaseAndExtent(s.anchorNode, index, s.focusNode, s.focusOffset);
        }
...
__vieb_custom_deleteToStart(__vieb_custom_getActiveElement());
